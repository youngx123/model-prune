...
class TrainingTask(LightningModule):
    ...
    def __init__(self, cfg, prune=None, sparsity_learning=False, evaluator=None):
            ...
            if metric > self.save_flag:
                ...
                if self.pruner is not None:
                    self.trainer.save_checkpoint(
                        os.path.join(best_save_path, "model_best-tp.ckpt")
                    )
                    self.save_model_state(
                        os.path.join(best_save_path, "nanodet_model_best-tp.pth")
                    )
                    # self.save_model_pth(os.path.join(best_save_path, "model_best-tp.pt"))

                    txt_path = os.path.join(best_save_path, "eval_results-tp.txt")
                else:
                    self.trainer.save_checkpoint(
                    ...
                    # self.save_model_pth(os.path.join(best_save_path, "model_best.pt"))
    ...
    def optimizer_step(
        ...
        # update params
        if (self.pruner is not None) and (self.sparsity_learning):
            print("sparsity learning")
            self.pruner.regularize(self.model)  # for sparsity learning
        # elif ((self.pruner is not None) and not self.sparsity_learning):
        #     self.pruner.step()
    ...
    def save_model_state(self, path):
        ...
        path2 = path.repalce(".pth", "_weight_averager_state.pth")
        path3 = path.repalce(".pth", "_model.pth")
        torch.save(self.weight_averager.state_dict(), path2)
